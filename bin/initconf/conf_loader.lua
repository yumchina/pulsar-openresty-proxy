---
--- Meta configuration JSON loader
--- Loading assigned configuration path first;
--- then loading default path configuration:
---   choosing /etc/orProxy.json first,if not exists,
---   then choosing prefix .. /conf/orProxy.json
--- @author Jacobs Lei
--- @since 2019-07-29
---
...
local CONF_INFERENCES = {
  ...
  wk_global_cache_dict_size = {typ = "string"},
  wk_stat_dashboard_data_dict_size = {typ = "string"},
  wk_lock_dict_size = {typ = "string"},
  wk_worker_process_events_dict_size = {typ ="string"},
  wk_health_checks_dict_size = {typ ="string"},
  ...
  wk_rate_limit_counter_dict_size = {typ = "string"},
}
...
local function load(conf_path,prefix)
  ...
  if not conf_path then
    conf_path = "/etc/orProxy.json"
    if not pl_path.exists(conf_path) then
      conf_path = prefix .. "/conf/orProxy.json"
    end
    if not pl_path.exists(conf_path) then
