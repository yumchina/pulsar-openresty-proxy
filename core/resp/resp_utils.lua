...
local html_template = '<html><head><title>OrProxy Error</title></head><body><h1>OrProxy Error</h1><p>error_code:%s,error_message:%s.</p></body></html>'
...
local _M = {
    ...
    errors = {
        ...
        GATEWAY_ERROR={CODE=20001,MESSAGE="[OrProxy]Internal error"},
        -- 3xxxx gateway rejected
        REQUEST_REJECTED={CODE=30001,MESSAGE="[OrProxy]Gateway reject the request"},

        --4xxxx service error
        SERVICE_NOT_FOUND={CODE=40004,MESSAGE="[OrProxy]Illegal Request, The host or api group had not been found"},
        SERVICE_UNAUTHORIZED={CODE=40001,MESSAGE="[OrProxy]Upstream service's unauthorized"},

        UPSTREAM_ERROR={CODE=50001,MESSAGE="[OrProxy]Upstream service is not available"}
    }
}
...
---send errors response to client by OrProxy
-- @parameters status_code HTTP STATUS CODE
-- @parameters error_code error code defined by OrProxy api gateway
...
function _M.say_upstream_interval_error_response(ngx, default_content_type, default_message)

    local resp_message = default_message;
    local resp_content_type = default_content_type;

    ngx_log(ERR,"upstream error OrProxy response,http status[" ..ngx.status .. "],http content-type[" .. resp_content_type .."],response message["..resp_message.."]")
